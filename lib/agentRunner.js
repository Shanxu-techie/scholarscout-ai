...
export function executeFilterScholarships(studentProfile, scholarshipList) {
  return scholarshipList.filter((s) => {
    ...
    if (s.fields && s.fields.length > 0) {
      const fieldOk = s.fields.some(
        (f) =>
          f.toLowerCase().includes(studentProfile.fieldOfStudy.toLowerCase()) ||
...
function dispatchTool(name, args) {
  switch (name) {
    ...
    case 'filterScholarships':
      return executeFilterScholarships(args.studentProfile, args.scholarshipList);
    case 'rankScholarships':
      return executeRankScholarships(args.filteredList, args.studentProfile);
    case 'explainMatch':
      return executeExplainMatch(args.scholarship, args.studentProfile, args.score);
    default:
...
export async function runScholarScoutAgent(studentProfile) {
  ...
  const messages = [
    ...
    { role: 'user', content: `Find scholarships for this student: ${JSON.stringify(studentProfile)}` }
  ];
